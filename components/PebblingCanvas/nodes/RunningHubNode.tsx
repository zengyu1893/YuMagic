...
export const RunningHubNode = React.memo(function RunningHubNode(props: NodeRendererProps) {
  ...
  const appName = (appInfo as any)?.webappName || appInfo?.title || '手动配置';
  ...
  // ID 输入框失焦 → 触发获取配置
  const handleIdBlur = () => {
    const id = mode === 'app' ? node.data?.webappId : node.data?.workflowId;
    if (id && onExecute) onExecute(node.id);
  };

  // ID 输入框变化 → 清除旧数据，准备切换应用
  const handleIdChange = (value: string) => {
    if (mode === 'app') {
      handleDataUpdate({ webappId: value, appInfo: undefined, nodeInputs: {}, coverUrl: undefined, error: undefined });
    } else {
      handleDataUpdate({ workflowId: value, workflowNodeList: [], nodeInputs: {}, coverUrl: undefined, error: undefined });
    }
  };
  ...
  return (
    <div className="w-full h-full flex flex-col">
          ...
          <div className="flex items-center gap-2 flex-1 min-w-0">
            ...
            <div className="flex flex-col min-w-0">
              ...
              <span className="text-[7px] truncate" style={{ color: isLightCanvas ? '#047857' : 'rgba(52,211,153,0.6)' }}>
                ID: {(mode === 'app' ? webappId : workflowId).slice(0, 12)}...
              </span>
            </div>
          </div>
          {/* 队列状态显示 */}
      ...
      {/* ID 输入区（手动模式：文本框输入，预设模式：下拉选择——这是唯一区别） */}
      <div className="px-3 py-2" style={{ backgroundColor: themeColors.nodeBg, borderLeft: `1px solid ${isLightCanvas ? 'rgba(16,185,129,0.3)' : 'rgba(16,185,129,0.3)'}`, borderRight: `1px solid ${isLightCanvas ? 'rgba(16,185,129,0.3)' : 'rgba(16,185,129,0.3)'}` }}>
        ...
        <input
          ...
          style={{
            ...
            border: `1px solid ${isLightCanvas ? 'rgba(255,215,0,0.3)' : 'rgba(255,215,0,0.3)'}`,
            color: themeColors.textPrimary
          }}
          placeholder={mode === 'app' ? '输入应用 ID...' : '输入工作流 ID...'}
          value={mode === 'app' ? webappId : workflowId}
          onChange={(e) => { e.stopPropagation(); handleIdChange(e.target.value); }}
          onBlur={handleIdBlur}
          onKeyDown={(e) => { if (e.key === 'Enter') handleIdBlur(); }}
          onMouseDown={(e) => e.stopPropagation()}
        />
        {!appInfo && (webappId || workflowId) && (
          <div className="text-center mt-1.5">
            <span className="text-[9px] text-zinc-500">输入后点击空白处自动获取配置</span>
          </div>
        )}
      </div>
          ...
          {isRunning && webappId && !appInfo && mode === 'app' ? (
            ...
            (() => {
              ...
              if (displayList.length === 0) return (
                <div className="text-center py-4 text-[11px]" style={{ color: themeColors.textMuted }}>{mode === 'app' ? '输入应用 ID 加载参数' : '输入工作流 ID 加载参数'}</div>
              );
